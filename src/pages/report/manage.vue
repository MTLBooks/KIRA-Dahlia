...
import { reportList, reportUpdateStatus } from '../../api/Report/ReportController'
...
const columns = computed<DataTableColumns<any>>(() => [
	...
	{ title: 'Actions', key: 'actions', render: (row) => (
		<NSpace>
			<NButton size="small" onClick={() => changeStatus(row.reportId, 'reviewing')}>Review</NButton>
			<NButton size="small" onClick={() => changeStatus(row.reportId, 'resolved')}>Resolve</NButton>
			<NButton size="small" onClick={() => changeStatus(row.reportId, 'dismissed')}>Dismiss</NButton>
		</NSpace>
	) }
])
...
async function load() {
	...
	loading.value = true
	try {
		const res = await reportList({ page: pagination.page, pageSize: pagination.pageSize, status: statusFilter.value || undefined })
		if (res.success) {
...
async function changeStatus(reportId: number, status: 'open'|'reviewing'|'resolved'|'dismissed') {
	try {
...
<template>
	<div class="container">
		...
		<NDataTable :columns="columns" :data="items" :pagination="false" :bordered="false" :rowKey="row => row.reportId" />
